...
st.markdown("**Fungsi/Kegunaan:** Memantau tren curah hujan historis dan prediksi jangka panjang. Berguna untuk pertanian, mitigasi banjir, dan monitoring musim hujan.")
...
# Tambahan: Temperature With Baseline Comparison
baseline_temp = df.groupby(df['Tanggal'].dt.month)['Tavg'].mean()
df['TempVsBaseline'] = df.apply(lambda r: r['Tavg'] - baseline_temp[r['Tanggal'].month], axis=1)
st.markdown("**Temperature With Baseline Comparison:** Menunjukkan deviasi harian dari rata-rata historis per bulan. Berguna untuk deteksi gelombang panas atau pendinginan ekstrem.")
fig_temp_baseline = px.line(df, x='Tanggal', y=['Tavg','TempVsBaseline'], labels={'value':'Temperature (°C)','variable':'Type'}, title="Temperature vs Baseline")
...
st.markdown("**Fungsi/Kegunaan:** Memantau kenyamanan cuaca dengan menggabungkan curah hujan, suhu, kelembaban, dan angin.")
latest = df.iloc[-1]
...
anomaly_pivot = df.pivot_table(index=df['Tanggal'].dt.year, columns=df['Tanggal'].dt.month, values='TempVsBaseline')
